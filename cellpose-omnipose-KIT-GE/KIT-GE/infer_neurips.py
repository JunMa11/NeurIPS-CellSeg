...
parser.add_argument('--cell_type', '-ct', nargs='+', default=['BFDP-NeurIPS-Cell'], help='Cell type(s) to predict')
parser.add_argument('--fuse_z_seeds', '-fzs', default=False, action='store_true', help='Fuse seeds in axial direction')
parser.add_argument('--model', '-m', default='BFDP-NeurIPS-Cell_GT_01_model_03', type=str, help='Name of the model to use')
...
for ct in args.cell_type:
    ...
    assert os.path.isfile(model), 'Model {} not found'.format(model)
    ...
    for subset in subsets:
        ...
        inference_2d_ctc(model=model,data_path=path_data / ct / subset,result_path=path_seg_results,device=device,batchsize=args.batch_size,args=inference_args,num_gpus=num_gpus)
